...
function initAnzic(){
    ...
    key_list = [];
    cur_code_list = [];
    //var view = document.getElementById("key_view");
    //var input = document.getElementById("key_input");
    //input.value = '';
    //view.innerHTML = "";

    $('#ulView').find('li').remove();
...
function show_anzic_details(code) {
    ...
    if (anzic_code_by_code[code] === undefined) {
        obj.innerHTML = `Not founded the information of ${code}.`;
    }
    var code_meta = anzic_code_by_code[code];
    var div = code_meta.division;
    var subdiv = code_meta.subdivision;
...
function fillAnzicInfo() {
    ...
    $('#anzic_div').innerHTML="";
    keys.forEach(key => {
        $('#anzic_div').append(`<option val="${key}">${key}</option>`);
    })
...
function fillAnzicSubdiv(div) {
    ...
    keys.forEach(key => {
        $('#anzic_subdiv').append(`<option val="${key}">${key}</option>`);
    })
...
function fillAnzicGroup(div, subdiv) {
    ...
    keys.forEach(key => {
        $('#anzic_group').append(`<option val="${key}">${key}</option>`);
    })
...
function fillAnzicClass(div, subdiv, group) {
    ...
    keys.forEach(item => {
        ...
        $('#anzic_class').append(`<option code="${code}">${key}</option>`);
    });
...
function search_anzic_code1(query, callback) {
    ...
    var endpoint_mpnet = `http://${ANZIC_URL}/search/${query}`;

    $.ajax({type: "GET", url: endpoint_mpnet, success: function(result){
        try {
            if (result.length){
...
function filterCandidate(result) {
    ...
    result.forEach( item => {
        ...
        var val = `${code}(${[div, sdiv, grp, cls].join('>')})`
        if (cnt == 0) {
            res['anzicCode'] = val;
        }
        else {
            res[`Candidate(${cnt})`] = val;
        }
        cnt += 1;
